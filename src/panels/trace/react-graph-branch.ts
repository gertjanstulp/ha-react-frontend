...
import { BRANCH_HEIGHT, SPACING } from "../../../homeassistant-frontend/src/components/trace/hat-graph-const";
 ...
 * @attribute track
 */
@customElement("react-graph-branch")
export class ReactGraphBranch extends LitElement {
    ...
    return html`
      ...
      <div id="branches">
        <svg id="lines" width=${this._totalWidth} height=${this._maxHeight}>
          ${this._branches.map((branch) => {
            if (branch.end || branch.notail) return "";
            return svg`
                    <path
                      class=${classMap({
                        track: branch.track,
...
declare global {
  interface HTMLElementTagNameMap {
    "react-graph-branch": ReactGraphBranch;
  }
}
